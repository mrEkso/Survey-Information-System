...
import com.example.oss.api.services.VoteValue.VoteValueService;
...
public class VoteServiceImpl implements VoteService {
    ...
    public Vote handleVote(UUID surveyId, UUID surveyOptionId, List<VoteValueDto> voteValues, User user) {
        ...
        fr.getSurveyRepository().save(survey);
    ...
    private void vote(Vote vote, User user) {
        ...
        surveyOptionDb.setVotes(surveyOptionDb.getVotes() + 1);
        fr.getSurveyOptionRepository().save(surveyOptionDb);
    ...
    private void voteWithValues(Vote vote, List<VoteValueDto> voteValues, User user) {
        ...
        for (VoteValueDto valueDto : voteValues) {
            ...
            fr.getSurveyOptionRepository().save(option);
        }
    }
    ...
    public void unvote(Survey survey, User user) {
        ...
        if (vote.getSurveyOption() != null) {
            SurveyOption surveyOption = vote.getSurveyOption();
            surveyOption.setVotes(surveyOption.getVotes() - 1);
            fr.getSurveyOptionRepository().save(surveyOption);
        } else
            vote.getVoteValues().forEach(voteValue -> {
                SurveyOption option = voteValue.getSurveyOption();
                option.setVotes(option.getVotes() - 1);
                fr.getSurveyOptionRepository().save(option);
            });
        survey.setVotesCount(Math.max(0, survey.getVotesCount() - 1));
        fr.getSurveyRepository().save(survey);
